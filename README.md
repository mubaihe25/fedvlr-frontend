...
- `src/pages/Home.tsx`：项目导览页，用一句话定位、三步流程、三个入口按钮和动态联邦拓扑图帮助快速理解项目。
- `src/pages/SystemMechanism.tsx`：系统机制页，以五层系统架构图为主视觉：数据层、客户端层、服务端层、安全层、展示层。
- `src/pages/AttackDefenseRange.tsx`：攻防工作台，承载普通/专家实验编排、训练过程监控、单次故事线分析、横向指标对比、实验档案库。
...
- 项目导览页动态联邦拓扑：服务器呼吸光、客户端轻微浮动、正常/攻击/防御飞线、客户端 hover 数据浮层。
- 系统机制页五层主架构图和一轮联邦推荐流程图。
- 攻防工作台五个 Tab 的正式流程：实验剧本、训练监控、单次故事线分析、指标对比、artifact 场景库。
- 实验编排支持普通模式（三攻三防）和专家模式（数据集、模型、轮数、采样、恶意比例、目标商品、攻击强度、防御算法、TopK/artifact 导出）。
- 运行监控页展示联邦拓扑飞线、客户端本地训练、更新/梯度上传、服务端聚合、恶意更新、防御过滤环、终端日志、状态摘要和实验摘要曲线。
- 单次分析页展示 V2.5 结果故事线：target rank `170 -> 3`、排名提升、Top50 未命中、目标商品轨迹、MIA AUC、交互候选还原 hit@50、安全聚合残差。
- 推荐列表默认请求 5 条，展开 15/50 条时再次请求 API limit 切片；目标商品出现时高亮，未出现时在目标轨迹中说明最终推荐未曝光。
- 横向对比接入场景摘要和模型能力矩阵，不展示推荐列表。
- 历史实验接入 `/showcase/scenarios` 实验档案库，支持 Amazon / KU / FedAvg / MMFedRAP / 投毒 / 隐私攻击 / 鲁棒防御筛选，点击场景切换当前工作台分析对象。
