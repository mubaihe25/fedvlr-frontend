...
每个剧本卡片展示实验目的、攻击模块、可选防御、观测指标、推荐数据集/模型和当前是否有 artifact 证据。专家参数不是独立页面，而是当前剧本下的展开区，包含数据集、模型、训练轮数、本地轮数、客户端采样比例、恶意客户端比例、目标商品、攻击强度、防御算法、保存 TopK 和导出审计 artifact。

剧本选择必须驱动完整工作流：自动更新专家参数和推荐场景，运行监控日志随剧本变化，单次分析优先展示当前剧本证据，横向对比说明当前在比什么，历史实验点击后切换场景并进入单次分析。
